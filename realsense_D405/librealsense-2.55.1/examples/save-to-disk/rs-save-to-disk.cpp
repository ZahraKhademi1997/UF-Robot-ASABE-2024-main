...
int main(int argc, char * argv[]) try {
    ...
    while (true) {  // Change this condition to stop based on your application's logic
        ...
        //         if (auto vf = frame.as<rs2::video_frame>()) {  // We can only save video frames as pngs
        //             if (!vf.is<rs2::depth_frame>()) {
        //                 vf = color_map.process(frame);  // Get an rgb image for the depth stream
        //             }

        //             std::stringstream png_file;
        //             png_file << "rs-save-to-disk-output-" << frame_count++ << ".png";
        //             stbi_write_png(png_file.str().c_str(), vf.get_width(), vf.get_height(),
        //                            vf.get_bytes_per_pixel(), vf.get_data(), vf.get_stride_in_bytes());
        //             std::cout << "Saved " << png_file.str() << std::endl;

        //             // std::stringstream csv_file;
        //             // csv_file << "rs-save-to-disk-output-" << frame_count - 1 << "-metadata.csv";
        //             // metadata_to_csv(vf, csv_file.str());
        //         }
        //     }
        if (current_time >= next_frame_time) {
            ...
            for (auto&& frame : frameset) {
                if (auto vf = frame.as<rs2::video_frame>()) {  // We can only save video frames as pngs
                    if (!vf.is<rs2::depth_frame>()) {  // Check if the frame is not a depth frame
                        std::stringstream png_file;
                        png_file << "rs-save-to-disk-output-" << frame_count++ << ".png";
                        stbi_write_png(png_file.str().c_str(), vf.get_width(), vf.get_height(),
                                       vf.get_bytes_per_pixel(), vf.get_data(), vf.get_stride_in_bytes());
            ...
            next_frame_time += std::chrono::seconds(3);  // Schedule the next frame capture
        }
    }
